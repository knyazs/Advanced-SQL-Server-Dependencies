...
namespace AdvancedSqlServerDependencies
{
    ...
    public static class Excel
    {
        ...
        public static SpreadsheetDocument CreateWorkbook(string fileName)
        {
            ...
            {
                MessageBox.Show(exception.Message, "Excel OpenXML basics", MessageBoxButton.OK, MessageBoxImage.Hand);
            }
        ...
        {
            string columnValue = intValue.ToString();
...
#else
            string columnValue = doubleValue.ToString(CultureInfo.InvariantCulture).Replace(",", ".");
#endif
        ...
        public static bool SetColumnWidth(Worksheet worksheet, int columnIndex, int width)
        {
            ...
            // Get the column
            column = columns.Elements<Column>().FirstOrDefault(item => item.Min == columnIndex);
            if (column == null)
            {
                return false;
            }
